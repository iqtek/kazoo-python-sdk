...
class Client(metaclass=RestClientMetaClass):
    ...
    called 'data' which is the last argument to the method. For example ::
        ...
        >>>client.update_account(account_id, {"name": "somename", "realm":"superfunrealm"})

    Dictionaries and lists will automatically be converted to their appropriate
    representation so you can do things like: ::

        >>>client.update_callflow(account_id, callflow_id, {"flow":{"module":"somemodule"}})
    ...
    correspondence is as follows. ::
        ...
        GET /accounts/{account_id}/callflows -> client.get_callflows(account_id)
        GET /accounts/{account_id}/callflows/{callflow_id} -> client.get_callflow(account_id, callflow_id)
        PUT /accounts/{account_id}/callflows/ -> client.create_callflow(account_id, data)
        POST /account/{account_id}/callflows/{callflow_id} -> client.update_callflow(account_id, data)
        DELETE /account/{account_id}/callflows/{callflow_id} -> client.delete_callflow(account_id, callflow_id)
    ...
    _hotdesks_resource = RestResource(
        "hotdesk",
    ...
    _menus_resource = RestResource(
        "menu",
        "/accounts/{account_id}/menus/{menu_id}"
    )
    
    _metaflow_resource = RestResource(
        "metaflow",
    ...
    _migration_resource = RestResource(
        "migration",
        "/accounts/{account_id}/migrations/{migration_id}",
        methods=['list', 'detail', 'update']
    )

    _notification_resource = RestResource(
        "notification",
    ...
    _rate_limits_resource = RestResource(
        "rate_limits",
    ...
    _recording_resource = RestResource(
        "recording",
        "/accounts/{account_id}/recordings/{record_id}",
        methods=["list", "detail"]
    )

    _registrations_resource = RestResource(
        "registration",
    ...
    def create_phone_number(self, account_id, phone_number):
        request = KazooRequest("/accounts/{account_id}/phone_numbers/{phone_number}",
                               method="put")
        return self._execute_request(request,
                                     account_id=account_id, phone_number=phone_number)

    def get_phone_number(self, account_id, phone_number):
        request = KazooRequest("/accounts/{account_id}/phone_numbers/{phone_number}",
                               method="get")
        return self._execute_request(request,
                                     account_id=account_id, phone_number=phone_number)

    def upload_media_file(self, account_id, media_id, filename, file_obj):
    ...
    def upload_phone_number_file(self, account_id, phone_number, filename, file_obj):
    ...
    def list_devices_by_owner(self, account_id, owner_id):
        request = KazooRequest("/accounts/{account_id}/devices",
                               get_params={"filter_owner_id": owner_id})
        request.auth_required = True

        return self._execute_request(request, account_id=account_id)

    def list_child_accounts(self, parent_account_id):
        request = KazooRequest("/accounts/{account_id}/children")
        request.auth_required = True
        return self._execute_request(request, account_id=parent_account_id)

    def delete_numbers_collection(self, account_id=None, del_data=None):
        if not account_id:
    ...
    def list_numbers_by_prefix(self, account_id=None, prefix_data=None):
        if not account_id:
            account_id = self.account_id
